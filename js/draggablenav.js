...
YAHOO.example.DDApp = {
    ...
    init: function() {
		...
		for ( i=0; i<ddc.length; i++ ) {
			...
			// grab all dragable modules in the container, based off class name "dragableli"
			var ddm = Dom.getElementsByClassName(container, 'li', container);
...
YAHOO.example.DDList = function(id, sGroup, config) {
	...
	var draghandle = Dom.getElementsByClassName('draghandle', 'span', id)[0].getAttribute('id');
	//alert(draghandle);
	this.setHandleElId(draghandle)
	
    this.logger = this.logger || YAHOO;
    var el = this.getDragEl();
    Dom.setStyle(el, "opacity", 0.95); // The proxy is slightly transparent
